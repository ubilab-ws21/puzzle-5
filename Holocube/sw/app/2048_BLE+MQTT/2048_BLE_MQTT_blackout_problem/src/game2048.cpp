...
if (deviceConnected) {
   ...
   if (RETURN == act_info->active)
    ...
    if (game.judge() == 1)
    {
       ...
       // mqtt.publish(topic, "'method': 'trigger', 'state': 'on'", retained);
        //mqtt.publish(topic, "game_win", retained);
        //if you win there will be a hint
        /*此处应该是退出的标志，当满足一定的数字之后，就要显示text/png来给下一个puzzel*/
       // hint();
       // app_exit();
    }
    else if (game.judge() == 2)
    {
        ...
        const char * topic = "Puzzle_5_status:";
        //bool retained = false;
        //mqtt.publish(topic, "'method': 'trigger', 'state': 'off', 'data': 'skipped'", retained);
...
APP_OBJ game_2048_app1 = {"EscapeRoom", &EscapeRoom, "", NULL,
